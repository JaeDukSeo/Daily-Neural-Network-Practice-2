...
for iter in range(num_epoch):
    ...
    for current_batch_index in range(0,len(train_data),batch_size):
        ...
        for ii in range(5):
            ...
            plt.subplot(3,9,17)
            plt.hist(train_white_Best[ii], bins='auto')
            plt.subplot(3,9,18)
            plt.hist(train_white_compare[ii], bins='auto')
            # ==== show histogram =====
            ...
            plt.subplot(3,9,26)
            plt.axis('off')
            sns.heatmap(np.cov(train_white_Best))
            plt.subplot(3,9,27)
            plt.axis('off')
            sns.heatmap(np.cov(train_white_compare))
            # ==== show heatmap =====
