...
training_data = training_data[0:number_of_trainin_images,:]

# preprocess (from: http://cs231n.github.io/neural-networks-2/ )
# training_data -= np.mean(training_data, axis = 0) # zero-center the data (important)
# cov = np.dot(training_data.T, training_data) / training_data.shape[0]
# U,S,V = np.linalg.svd(cov)
# training_data = np.dot(training_data, U) # decorrelate the data
# training_data = training_data / np.sqrt(S + 1e-5)

# hyper
num_epoch = 800
learning_rate = 0.0001
batch_size = 200; print_size = 10
...
l1 = FNN(compress_size,784,act=tf_sigmoid,d_act=d_tf_sigmoid)
...
reconstruction_cost = tf.reduce_mean(
...
grad0,gra0_up = l0.backprop(grad1,l2_regularization=True)
grad_update = grad1_up + gra0_up
...
with tf.Session() as sess:
    ...
    training_data_reshape = np.reshape(training_data_show,(16,28,28))
    ...
    trained_w1_data_reshape = np.reshape(trained_w1.T,(16,28,28))
    fig=plt.figure(figsize=(8, 8))
    columns = 4; rows = 4
    for i in range(1, columns*rows +1):
        fig.add_subplot(rows, columns, i)
