...
addpath(genpath('/media/disk/vista/Papers/MODVIS_2016/MODVIS_linear/VisRes_16/code/'))
path_result = '/media/disk/vista/Papers/MODVIS_2016/MODVIS_linear/VisRes_16/code/';

lados = [16 20 32 50 100];
i = 3;          % SIZEblock
factor = 4;     % FACTsampling
lado = lados(i);

x = [];
for i = 1:13
    if factor == 1
       load([path_result,'data_',num2str(lado),'_im_',num2str(i)])
    else
       load([path_result,'data_',num2str(factor),'x_',num2str(lado),'_im_',num2str(i)])  
    end
    x = [x xx];
end
...
X = xw;
  ...
  estimate( WM, iWM, [path_result,['tica_',num2str(factor),'x_',num2str(lado),'_image.mat']], p );
