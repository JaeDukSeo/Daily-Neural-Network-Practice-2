clear all
% Fig 6 of the paper
% Up left, an example of the distribution of oriented TICA mechanisms in 
% the retinal space, $S_{ori}$, for certain orientation. Each point 
% corresponds to the center of a mechanism tuned to 135^o 22^o.
% Down left is the histogram of the above set.
% Middle column, an example of a 2-D random sample uniformly distributed, $S_{rand}$,
% with the same number of points than $S_{ori}$, and its corresponding
% histogram. Right column, an example of Cartesian grid ($S_{cart}$) and 
% its histogram. The grid of the top figures shows the bins used to built
% the histograms. In histograms, black corresponds to empty bins while more 
% populated bins are depicted in white.
%close all
...
load([ path_read '\sortu_2x_32_A.mat'])
fs=Vfs(6); 
