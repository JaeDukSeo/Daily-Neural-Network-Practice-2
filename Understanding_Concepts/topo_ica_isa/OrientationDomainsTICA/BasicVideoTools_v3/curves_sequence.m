function [s,vr] = curves_sequence(ne,nt,fse,fst,fm,fM,anch_ang,orient,fm2,fM2,v,interpol)

% CURVES_SEQUENCE generates a discrete sequence of random curvy patterns that moves at certain uniform speed 
% Sampling frequencies, size of the sequence, spatial frequency of the pattens and speed are controlled.
%
...
% Random curvy pattern (Winawer et al. PLoS Comp. Biol. 2013) depends on:
%      . Input noise: 
%           Spatial frequency: fm, fM (in cycl/deg)
%           Orientation: orient (in degrees)
%           Orientation width: angul_wdth (in degrees).
%           (this noise gets binarized afterwards to lead to curves)
%      . Spatial frequency of filter applied to the curves (in degrees):
%           fm2, fM2 (in cycl/deg)
% Speed:
%      v = [vy vx] in deg/sec 
...
%
% SYNTAX: [ s, v_true] = curves_sequence(nx,nf,fsx,fst,fm,fM,angul_wdth,orient,fm2,fM2,[vy vx],interpol(0/1)?);
%
...
s1 = curvy_pattern(l,fse,fm,fM,orient,anch_ang,fm2,fM2);
