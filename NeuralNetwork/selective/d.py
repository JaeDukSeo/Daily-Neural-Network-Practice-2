...
class CNN():
    ...
    def backprop(self,gradient,learning_rate_change,stride=1,padding='SAME',mom=False,adam=False,awsgrad=False):
...
grad5,grad5_up = l5.backprop(grad6,learning_rate_change=learning_rate_change,awsgrad=True)
...
grad1,grad1_up = l1.backprop(grad2,learning_rate_change=learning_rate_change,awsgrad=True)
...
with tf.Session() as sess:
    ...
    for iter in range(num_epoch):
        ...
        random_drop1 = np.random.uniform(low=0.7,high=0.8)
        random_drop2 = np.random.uniform(low=0.8,high=0.9)
        random_drop3 = np.random.uniform(low=0.9,high=1.0)
