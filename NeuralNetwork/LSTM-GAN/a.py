...
all_brain_data = np.zeros((20,192,128,128))
for current_brain in range(len(all_brain_data)):
    all_brain_data[current_brain] = zoom(nib.load(lstFilesDCM[current_brain]).get_fdata().T ,(1, 1/2, 1/2))
...
for dirName, subdirList, fileList in os.walk(PathDicom):
    for filename in fileList:
        if ".nii.gz" in filename.lower() and  'brainmask' in filename.lower() :  # check whether the file's DICOM
            lstFilesDCM.append(os.path.join(dirName,filename))
all_mask_data = np.zeros((20,192,128,128))
for current_brain in range(len(all_mask_data)):
    all_mask_data[current_brain] = zoom(nib.load(lstFilesDCM[current_brain]).get_fdata().T ,(1, 1/2, 1/2))
...
np.save('all_brain_data_128.npy', all_brain_data) 
np.save('all_mask_data_128.npy', all_mask_data) 
sys.exit()
