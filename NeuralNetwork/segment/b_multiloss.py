...
mid_cnn1 = CNN(3,512,1,tf_sigmoid,d_tf_sigmoid)

dl2 = CNN_Trans(3,256,512)
mid_cnn2 = CNN(3,256,1,tf_sigmoid,d_tf_sigmoid)

dl3 = CNN_Trans(3,128,256)
mid_cnn3 = CNN(3,128,1,tf_sigmoid,d_tf_sigmoid)
...
y1 = tf.image.resize_images(y, [16, 16])
y2 = tf.image.resize_images(y, [32, 32])
y3 = tf.image.resize_images(y, [64, 64])
...
mid_ouput1  = mid_cnn1.feedforward(dlayer1,padding='SAME')

dlayer2 = dl2.feedforward(dlayer1,stride=2,padding='SAME')
mid_ouput2  = mid_cnn2.feedforward(dlayer2,padding='SAME')

dlayer3 = dl3.feedforward(dlayer2,stride=2,padding='SAME')
mid_ouput3  = mid_cnn3.feedforward(dlayer3,padding='SAME')

final_output = final_cnn.feedforward(dlayer3,padding='SAME')

# calculate the loss
cost_mid_1 = tf.reduce_mean(tf.square(mid_ouput1-y1)) * 0.5
cost_mid_2 = tf.reduce_mean(tf.square(mid_ouput2-y2)) * 0.5
cost_mid_3 = tf.reduce_mean(tf.square(mid_ouput3-y3)) * 0.5
...
dgrad3_mid,dgrad3_mid_up = mid_cnn3.backprop(mid_ouput3-y3,padding='SAME')
dgrad3,dgrad3_up = dl3.backprop(final_grad + dgrad3_mid,stride=2,padding='SAME')

dgrad2_mid,dgrad2_mid_up = mid_cnn2.backprop(mid_ouput2-y2,padding='SAME')
dgrad2,dgrad2_up = dl2.backprop(dgrad3 + dgrad2_mid,stride=2,padding='SAME')

dgrad1_mid,dgrad1_mid_up = mid_cnn1.backprop(mid_ouput1-y1,padding='SAME')
dgrad1,dgrad1_up = dl1.backprop(dgrad2 + dgrad1_mid,stride=1,padding='SAME')
...
grad_update = final_grad_up + \
              dgrad3_mid_up + dgrad3_up + \
              dgrad2_mid_up + dgrad2_up + \
              dgrad1_mid_up + dgrad1_up + \
