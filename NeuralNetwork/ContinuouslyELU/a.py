...
def tf_celu(x,alpha=2.0 ):
    mask_greater = tf.cast(tf.greater_equal(x,0),tf.float16) * x
    mask_smaller = tf.cast(tf.less(x,0),tf.float16) * x
    middle = alpha * ( tf.exp(tf.divide(mask_smaller,alpha)) - 1)
    return middle + mask_greater
def d_tf_celu(x,alpha=2.0):
    mask_greater = tf.cast(tf.greater_equal(x,0),tf.float16) 
    mask_smaller = tf.cast(tf.less(x,0),tf.float16) * x
...
final_reshape = tf.reshape(layer5,[batch_size,-1])
