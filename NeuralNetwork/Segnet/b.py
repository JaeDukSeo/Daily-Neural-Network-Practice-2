...
def encode_segmap(mask,num_class=20):
    ...
    for label in get_pascal_labels_pixel():
        coordinate = np.where(np.all(mask == label, axis=-1))
        label_mask[coordinate[0],coordinate[1],mask_index ] = 1
...
train_labels = np.zeros(shape=(NUMBER_OF_IMAGE,128,128,20))
...
for file_index in range(len(train_images)):
    ...
    train_labels[file_index,:,:,:]     = encode_segmap(train_labels_RGB[file_index,:,:,:])
