...
#
# Main differences:
# - Use the standard momentum optimizer with a fixed learning rate schedule rather than learning rate annealing.
...
def run_model(data, image_shape, label_count):
  ...
  with graph.as_default():
    ...
    K = 2
    current = block_stack(current, 16,   int(16*K), 1, 4, is_training)
    current = block_stack(current, int(16*K), int(32*K), 2, 4, is_training)
    current = block_stack(current, int(32*K), int(64*K), 2, 4, is_training)
  ...
  with tf.Session(graph=graph) as session:
    ...
    for epoch in range(1, 1+90):
      if int(epoch) == 30: learning_rate = 0.01
      if int(epoch) == 60: learning_rate = 0.001
      
      for batch_idx in range(batch_count):
        batch_data = batches_data[batch_idx]
        batch_labels = batches_labels[batch_idx]
        batch_res = session.run([ train_step, cross_entropy, accuracy ],feed_dict = { xs: batch_data, ys: batch_labels, lr: learning_rate, is_training: True })
        print("Current Iter : ",epoch,' Acc: ', batch_res[2],end='\r')

      # save_path = saver.save(session, 'resnet_%d.ckpt' % epoch)
      test_results = run_in_batch_avg(session, [ cross_entropy, accuracy ], [ xs, ys ], feed_dict = { xs: data['test_data'], ys: data['test_labels'], is_training: False })
      print('\n------------------------',epoch, batch_res[1:], test_results)
...
def run():
  ...
  run_model(data, 3072, label_count)
