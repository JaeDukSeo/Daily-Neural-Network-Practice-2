...
class FNN():
    ...
    def backprop(self,gradient=None,indirect=False):
        grad_part_1 = gradient
        grad_part_2 = self.d_act(self.layer)
        grad_part_3 = self.input 

        if indirect:
            grad_added_d = tf.matmul(grad_part_1,self.w)
        else:
...
grad2,grad2_up = l2.backprop(grad1,indirect=True)
