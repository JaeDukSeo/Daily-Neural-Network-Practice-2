...
def train_autoencoder():
    ...
    encoder_decoder_update = theano_funcs.create_encoder_decoder_func(layers, apply_updates=True)
    print('  discriminator')
    discriminator_update = theano_funcs.create_discriminator_func(layers, apply_updates=True)
    print('  generator')
    generator_update = theano_funcs.create_generator_func(layers, apply_updates=True)
    ...
    try:
        for epoch in range(1, max_epochs + 1):
            ...
            for train_idx in get_batch_idx(X_train.shape[0], batch_size):
                ...
                # sample from p(z)
                pz_train_batch = np.random.uniform(low=-2, high=2,size=(X_train_batch.shape[0], 2)).astype(np.float32)
            ...
            for test_idx in get_batch_idx(X_test.shape[0], batch_size):
                ...
                test_reconstruction_losses.append(test_batch_reconstruction_loss)
                test_discriminative_losses.append(test_batch_discriminative_loss)
                test_generative_losses.append(test_batch_generative_loss)

            test_reconstruction_losses_mean = np.mean(test_reconstruction_losses)
            test_discriminative_losses_mean = np.mean(test_discriminative_losses)
