"""
Code taken from https://github.com/openai/InfoGAN/blob/master/infogan/misc/distributions.py
...
class Distribution(object):
    ...
    #def kl_prior(self, dist_info): #TODO
    #    return self.kl(dist_info, self.prior_dist_info(dist_info.values()[0].get_shape()[0]))

    def kl_prior(self, dist_info, batch_size):
        return self.kl(dist_info, self.prior_dist_info(batch_size))
...
class Gaussian(Distribution):
    ...
    def logli(self, x_var, dist_info, drop_const=True):
        mean = dist_info["mean"]
        stddev = dist_info["stddev"]
        epsilon = (x_var - mean) / (stddev + TINY)
        if not drop_const: # modified
            return tf.reduce_sum(
                - tf.log(stddev + TINY) - 0.5 * tf.square(epsilon),
                axis=1,
            )
        else:
            return tf.reduce_sum(
    ...
    def prior_dist_info(self, batch_size):
        ...
        # code modification:
        if self._stddev:
            stddev = tf.multiply(self._stddev, stddev)
...
class Bernoulli(Distribution):
    ...
    def cross_entropy(self, q, p):
        p_p = p["p"]
        q_p = tf.clip_by_valueq(q["p"], TINY, 1 - TINY)
        return - tf.reduce_sum(p_p * tf.log(q_p) + (1 - p_p) * tf.log(1 - q_p),
                               axis=1)
...
class Deterministic(Distribution):
    ...
    def logli(self, x_var, dist_info):
        z = dist_info["z"]
        return tf.reduce_sum(tf.log(tf.equal(x_var, z) + TINY),
                             axis=1)
