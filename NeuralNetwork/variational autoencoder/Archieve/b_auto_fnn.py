...
# define class here
el1 = FNN(28*28*1,500,tf_elu,d_tf_elu)
el2 = FNN(500,250,tf_elu,d_tf_elu)
el3 = FNN(250,3,tf_atan,d_tf_atan)

dl1 = FNN(3,250,tf_elu,d_tf_elu)
dl2 = FNN(250,500,tf_elu,d_tf_elu)
dl3 = FNN(500,784,tf_sigmoid,d_tf_sigmoid)

# graph
x = tf.placeholder(shape=[None,28,28,1],dtype=tf.float32,name="input")
x_reshape = tf.reshape(x,[batch_size,-1])
elayer1 = el1.feedforward(x_reshape)
elayer2 = el2.feedforward(elayer1)
elayer3 = el3.feedforward(elayer2)

dlayer1 = dl1.feedforward(elayer3)
dlayer2 = dl2.feedforward(dlayer1)
dlayer3 = dl3.feedforward(dlayer2)

final_output = tf.reshape(dlayer3,[batch_size,28,28,1])
reconstr_loss = -tf.reduce_sum(x_reshape * tf.log(1e-10 + dlayer3)+ (1-x_reshape) * tf.log(1e-10 + 1 - dlayer3))
cost = reconstr_loss

auto = tf.train.AdamOptimizer(learning_rate=learning_rate).minimize(cost)

# log_loss_back = (dlayer3 - x_reshape)/(dlayer3*(1-dlayer3))

# dgrad3,dgrad3_up = dl3.backprop(log_loss_back)
# dgrad2,dgrad2_up = dl2.backprop(dgrad3)
# dgrad1,dgrad1_up = dl1.backprop(dgrad2)
...
with tf.Session() as sess:
    ...
    for iter in range(num_epoch):
        ...
        for batch_size_index in range(0,len(train_batch),batch_size):
            ...
            sess_result = sess.run([cost,auto],feed_dict={x:current_batch})
