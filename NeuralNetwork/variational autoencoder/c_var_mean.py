...
el1 = CNN(3,1,256)
el2 = CNN(3,256,512)
el_mean = FNN(7*7*512,3,tf_iden,d_tf_iden)
el_std  = FNN(7*7*512,3,tf_iden,d_tf_iden)
...
eps = tf.random_normal((batch_size, 3), 0, 1, dtype=tf.float32)
z = elayer3_mean + tf.sqrt(tf.exp(elayer3_std)) * eps

# edcoder
dlayer1 = dl1.feedforward(z)
...
# calculate the loss
reconstr_loss = tf.reduce_sum(tf.square(final_output-x)*0.5)
latent_loss = 0.5 * tf.reduce_sum(tf.square(elayer3_mean) + tf.square(elayer3_std) - tf.log(tf.square(elayer3_std)+1e-10) - 1,1)
...
with tf.Session() as sess:
    ...
    test_latent = sess.run(z,feed_dict={x:test_batch[:batch_size,:,:,:]})
    for iii in range(batch_size,len(test_batch),batch_size):
        temp = sess.run(z,feed_dict={x:test_batch[iii:batch_size+iii,:,:,:]})
