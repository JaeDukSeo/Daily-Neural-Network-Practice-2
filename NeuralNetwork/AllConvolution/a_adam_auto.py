...
reg = tf.nn.l2_loss(l1w) + tf.nn.l2_loss(l2w) + tf.nn.l2_loss(l3w) + \
...
with tf.Session() as sess:
    ...
    for iter in range(num_epoch):
        ...
        if iter % print_size==0:
            print("\n---------- Learning Rate : ", learning_rate * (1.0/(1.0+learning_recy*iter)) )
